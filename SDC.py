...
class SDC(nn.Module):
    def __init__(self):
        ...
        self.EE_l = Extract_Edge(3, 3)

        self.backbone_hsi = Conv_Feature(in_channel=opt.pca_components)
        self.backbone_lidar = Conv_Feature(in_channel=3)
        self.neck_hsi = FPNBlock(3)
        self.neck_lidar = FPNBlock(3)
        ...
        self.edge_conv = nn.Conv2d(3, 3, 1, 1, 0)
    ...
    def forward(self, img_hsi, img_lidar):
        ...
        x_common_list = []
        for i in range(len(hsi_feature) - 1):
